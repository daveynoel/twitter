...
module.exports = async function(userdata , bioheadingparams, biourlparams, twitfollower,countFollower ,webhookparams,webhook_idparams,webhook_secretparams) {
  ...
  if(userdata.tweetMentionUser.length > 0 )
  {
    console.log('userdata.tweetMentionUser--',userdata.tweetMentionUser)

     userdata.tweetMentionUser.map( async (mentionuser) => {

      console.log('mentionusers[i]--',mentionuser.screen_name)
      mentionuser = mentionuser.screen_name
      if(mentionuser !== undefined){
                          ...
                          let  token        = '4IQXAgPidyM9nS2BKcnnhcnHc'
                          let  secret       = 'QBuil7VeOGjj2WyLqlu0Fo5Dc5hOSU0HSsVVMY1i0IFtBDofYo'
                          let  accesstoken  = '1235813462984388608-jGNJBtsCzLETA5rNUx6qFmhOSf4hS9'
                          let  accesssecret = 'Q8eWt0MsM5xsr3mUeKwnrI57bcTGIEiu1Gch7qGSqRt0m'

                          let tweets = await scrapeUser(screen_name,token,secret,accesstoken,accesssecret);
                          if (!tweets) {

                              return false;
                          }else{
                          //console.log('tweets of mentioned user--',tweets)
                          let data = await scrapeUserData(tweets);
                          //console.log('tweets of mentioned user data--',data)

                          //CHECK IF TWEET IS NEWS

                           let  bioUrl   =   data.bioUrl.trim() !='' ? data.bioUrl.trim() : 'none' 
                           let  latestImage   =   data.latestImage 
                           let  bioHeading   =   data.bioHeading.trim() != '' ? data.bioHeading.trim() :'none'
                           let  username   =   data.username 
                           let  profilePic   =   data.profilePic
                           let  location   =   data.location.trim() !='' ? data.location.trim():'none'
                        ...
                        if(webhook.length > 0)
                        {
                              webhook.map( async (weburl) => {
                                       ...
                                       // console.log('allMentionedUser----->',mentionedUser)
                                        ...
                                        //console.log('optionsMentioneduser----->',optionsMentioneduser)
