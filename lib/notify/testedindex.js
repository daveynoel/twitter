...
module.exports = async function(userdata , bioheadingparams, biourlparams, twitfollower,countFollower ,webhookparams,webhook_idparams,webhook_secretparams) {
    ...
    for(let i = 0; i<= mentionurls.length; i++)
    {
      if(mentionurls[i] !== undefined){
        ...
        inviteUrlDetected = mentionurls[i].indexOf("discord.gg") > -1 ?
         ...
         urlDetected = urlDetected +'([t.co]('+userdata.urlLink+')) - ' + mentionurls[i]+ "\n" 
  ...
  let allMentionedUser = [
    {
      author: {
        name: `${userdata.username} - ${userdata.follower}`,
...
if(mentionurls.length > 0 )
{
  ...
  for(let i = 0; i<= mentionurls.length; i++)
  {
    if(mentionurls[i] !== undefined){
      ...
      if(mentionurls[i].indexOf("pastebin.com") > -1 ){ 
        
        let formatedUrlKey = mentionurls[i].split('/')[3]
     ...
     // https://hastebin.com/raw/qogegutesi
      ...
      if(mentionurls[i].indexOf("hastebin.com") > -1 ){
        console.log("urldetected---",mentionurls[i])
      ...
      if(mentionurls[i].indexOf("ghostbin.co") > -1 ){
      console.log("urldetected---",mentionurls[i])
 
      //https://ghostbin.co/paste/89mt3
        let formatedUrlKey = mentionurls[i].split('/')[4]
     ...
     // https://ghostbin.co/paste/89mt3/raw
      ...
      if(mentionurls[i].indexOf("docs.google.com") > -1 ){
       // console.log("urldetected---",mentionurls[i])
   

         await fetch(mentionurls[i])
       ...
       Jimp.read(mentionurls[i], async function(err, image) {
...
if(biourlparams!== null){
    ...
    biourlparamsFields.push( {
      name: 'Bio Changed Detected',
      value: `[https://${biourlparams}](https://${biourlparams})`,
      inline: true
    });
